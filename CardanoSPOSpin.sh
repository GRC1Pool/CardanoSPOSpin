...
if [[ "${PROJECT_ID:0:7}" != "mainnet" && "${PROJECT_ID:0:7}" != "testnet" && "$#" -ge 1 ]]; then
...
if ((${#HASASSETOFPOLICY[@]})); then
        IFS=', ' read -r -a HASASSETOFPOLICYARRAY <<< "$HASASSETOFPOLICY"
fi

if ((${#HASASSET[@]})); then
        IFS=', ' read -r -a HASASSETARRAY <<< "$HASASSET"
fi
...
if ((${#POOL[@]})); then
  ...
  # v1.1
  TOTALASSETS=()
  if ((${#HASASSETOFPOLICYARRAY[@]})); then
        echo -n "Creating asset map..."
        for policyid in "${HASASSETOFPOLICYARRAY[@]}"; do
                reslts=100
                whatpage=0
                ((DEBUG==1)) && echo -n "policyid [${policyid}] pages: "
                while (( $(bc <<<"${reslts} >= 100") )); do
                        ((DEBUG==1)) && echo -n "$whatpage "
                        TMPASSSTR=$(curl --connect-timeout 5 -s -H "project_id: ${PROJECT_ID}" https://${BFHOST}/api/v0/assets/policy/${policyid}?page=$whatpage)
                        reslts=$(echo "${TMPASSSTR}" | jq length)
                        readarray -t -O"${#TOTALASSETS[@]}" TOTALASSETS < <(jq -rc '.[].asset' <<< $TMPASSSTR)
                        : $((whatpage++))
                done
                echo
        done
  fi
  if ((${#HASASSETARRAY[@]})); then
        echo -n "Adding assetIDs..."
        for assetid in "${HASASSETARRAY[@]}"; do
                TOTALASSETS+=(${assetid})
        done
        echo "Done"
  fi
        ...
        for items in "${TMPARRAY[@]}"; do
                ...
                ((bypass==0)) && {
                   if ((${#TOTALASSETS[@]})); then
                        ...
                        whatpage=0
                        while (( $(bc <<<"${reslts} >= 100") )); do
                                UNITSSTR=$(curl --connect-timeout 5 -s -H "project_id: ${PROJECT_ID}" https://${BFHOST}/api/v0/accounts/$TMPADDRESS/addresses/assets?page=$whatpage)
                                reslts=$(echo "${UNITSSTR}" | jq length)
                                readarray -t -O"${#TOTALUNITS[@]}" TOTALUNITS < <(jq -rc '.[].unit' <<< $UNITSSTR)
                                : $((whatpage++))
                        done
                        ArrContains TOTALASSETS TOTALUNITS && {
                                bypass=0 
                                v11str=", [asset owner]"
                        } || {
                                bypass=1
                                v11str=", [missing asset]"
                        }
                   fi
                }  
                # v1.1 --
                ...
                ((DEBUG==1)) && {
                 ((bypass==0)) && {
                        echo "Candidate [$candidates], Stake Address: [$TMPADDRESS], Live Stake: [${TMPLIVESTAKE}]${v11str}";
                        candidates=$((candidates+1))
                  } || { 
                        echo "EXCLUDING Stake Address: [$TMPADDRESS], Stake: [${TMPLIVESTAKE}]${v11str}";
                  }
                }
